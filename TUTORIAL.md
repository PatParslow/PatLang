...
`portfolio/index.html` is a self-contained showcase: each example displays
its PatLang source, runs the compiled WebAssembly *in your browser* (inline
WASI shim, no network), self-reports timings via `now_ms()`, and shows the
native transcript captured at build time — plus the compiler's own metrics
measured on its own source. Bigger examples include a point-of-sale
(events + OO + logic facts), a test framework (unit, event-driven
integration, and Gherkin features — `lib/test.patlang`) running its suite
in-browser, `patbuild` (a goal-oriented build system), and `flowgraph`
(control-flow graphs rendered as SVG from the compiler's own IR —
`portfolio/flowgraph.html`) — plus a **live playground**: the self-hosted
compiler compiled to WASM, so visitors edit PatLang in a syntax-highlighted
editor and compile+run it in their browser via `run_ir` (the IR executes on
the runtime VM — no rustc needed). The page is generated by PatLang
