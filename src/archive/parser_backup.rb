...
class Parser
  ...
  def eat(token_type)
    if @current_token.nil?
      error("Expected #{token_type}, got nil")
      return
    end

    # Handle AmbiguousToken - try to resolve to expected type
    if @current_token.ambiguous?
      resolved_token = @current_token.resolve_to(token_type)
      if resolved_token
        # Success! Replace current token with resolved token and advance
        @tokens[@current_token_index] = resolved_token
        @current_token = resolved_token
        advance
      else
        # Failed to resolve - provide helpful error message
        possible_types = @current_token.possible_types.join(', ')
        error("Expected #{token_type}, but ambiguous token can only be: #{possible_types}")
      end
    # Handle regular Token
    elsif @current_token.type == token_type
      advance
    else
  ...
  def assignment
    ...
    if token.type == Token::TOKEN_TYPES[:IDENTIFIER]
      ...
      # Try to resolve as IDENTIFIER for assignment
      resolved = token.resolve_to(Token::TOKEN_TYPES[:IDENTIFIER])
      if resolved
  ...
  def is_assignment?
    ...
    # Check if current token can be an identifier (regular or ambiguous)
    can_be_identifier = @current_token.type == Token::TOKEN_TYPES[:IDENTIFIER] ||
                       (@current_token.ambiguous? && @current_token.possible_types.include?(Token::TOKEN_TYPES[:IDENTIFIER]))
    
    return false unless can_be_identifier
  ...
  def parse_function_definition
    ...
    # Handle optional 'a' - could be ambiguous token or regular A token
    if @current_token
      if @current_token.ambiguous?
        # Try to resolve as A token first, if that fails, continue without consuming
        resolved = @current_token.resolve_to(Token::TOKEN_TYPES[:A])
        if resolved
          @tokens[@current_token_index] = resolved
          @current_token = resolved
          advance  # consume the 'a'
        end
      elsif @current_token.type == Token::TOKEN_TYPES[:A]
        eat(Token::TOKEN_TYPES[:A])
      elsif @current_token.type == Token::TOKEN_TYPES[:IDENTIFIER] && @current_token.value == "a"
        eat(Token::TOKEN_TYPES[:IDENTIFIER])
      end
    end
