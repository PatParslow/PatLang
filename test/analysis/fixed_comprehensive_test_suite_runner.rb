...
# FIXED Enhanced Comprehensive Test Suite Runner
# Addresses path resolution and nil value propagation issues
...
class FixedComprehensiveTestSuiteRunner
  def initialize
    # FIXED: Ensure we're working from project root
    @project_root = Dir.pwd
    @base_path = File.join(@project_root, 'test')
    @start_time = Time.now
    
    # FIXED: Initialize discovery_stats with default values to prevent nil errors
    @results = {
      'summary' => {},
      'categories' => {},
      'test_files' => {},
  ...
  def run_comprehensive_suite
    log_header("🚀 FIXED COMPREHENSIVE TEST SUITE RUNNER")
    log_info("Started at: #{@start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    log_info("Project root: #{@project_root}")
    log_info("Test base path: #{@base_path}")
    ...
    begin
      Timeout::timeout(@total_timeout) do
        # Enhanced dynamic test discovery with robust error handling
  ...
  def discover_all_test_files
    log_info("🔍 Enhanced dynamic test discovery with robust error handling...")
    ...
    begin
      ...
      @results['discovery_stats']['total_files_found'] = all_test_files.length
      
      # Apply refined exclusion patterns
      refined_exclusions = [
      ...
      @results['discovery_stats']['excluded_files'] = excluded_files.length
      @results['discovery_stats']['legitimate_test_files'] = legitimate_test_files.length
      
      # FIXED: Robust categorization with error handling
      @test_categories = categorize_test_files_safely(legitimate_test_files)
      
      # Update final discovery stats
      @results['discovery_stats']['categories_discovered'] = @test_categories.keys.length
      ...
      improvement = @results['discovery_stats']['legitimate_test_files'] - @results['discovery_stats']['previous_discovery_count']
      log_info("📈 Improvement: +#{improvement} additional test files discovered")
      log_separator
      
    rescue => e
      log_error("❌ Test discovery failed: #{e.class}: #{e.message}")
      # Ensure we have fallback values
      @test_categories = {}
      @results['discovery_stats']['error'] = "#{e.class}: #{e.message}"
    end
  end

  def categorize_test_files_safely(test_files)
    categories = Hash.new { |h, k| h[k] = [] }
    
    test_files.each do |file|
      begin
        # FIXED: Safe relative path calculation
        relative_path = get_safe_relative_path(file)
        ...
        log_warning("⚠️  Failed to categorize #{File.basename(file)}: #{e.message}")
        categories['uncategorized'] << file
      end
    end
  ...
  def get_safe_relative_path(file)
    begin
      Pathname.new(file).relative_path_from(Pathname.new(@base_path)).to_s
    rescue => e
      log_warning("⚠️  Path resolution failed for #{file}, using basename")
      File.basename(file)
    end
  end
  ...
  def run_single_test_file(test_file, category)
    ...
    relative_path = get_safe_relative_path(test_file)
  ...
  def run_isolated_test(test_file)
    ...
    script_path = File.join(@project_root, 'temp_fixed_test_runner.rb')
  ...
  def create_isolated_test_script(test_file)
    <<~RUBY
      # Fixed isolated test runner script
      begin
        # Set up load paths from project root
        ...
        helper_paths = [
          File.join('#{@base_path}', 'helpers', 'test_helper.rb'),
          File.join('#{@base_path}', 'test_helper.rb')
        ]
  ...
  def save_comprehensive_report
    log_info("💾 Saving fixed comprehensive test report...")
    
    # Save JSON report
    json_report_path = File.join(@project_root, 'FIXED_COMPREHENSIVE_TEST_SUITE_REPORT.json')
    File.write(json_report_path, JSON.pretty_generate(@results))
    
    # Save human-readable report
    text_report_path = File.join(@project_root, 'FIXED_COMPREHENSIVE_TEST_SUITE_REPORT.md')
  ...
  def generate_final_markdown_report
    report = []
    report << "# Fixed Comprehensive Test Suite Report"
    report << ""
  ...
  def display_final_summary
    log_separator
    log_header("📊 FIXED COMPREHENSIVE TEST SUITE SUMMARY")
    ...
    log_separator
    log_info("📄 Fixed comprehensive reports saved:")
    log_info("   - FIXED_COMPREHENSIVE_TEST_SUITE_REPORT.json")
    log_info("   - FIXED_COMPREHENSIVE_TEST_SUITE_REPORT.md")
...
# Main execution
if __FILE__ == $0
  runner = FixedComprehensiveTestSuiteRunner.new
  ...
  begin
    ...
    discovery_stats = results['discovery_stats'] || {}
    puts
    puts "🎉 FIXED TEST DISCOVERY COMPLETE!"
    puts "   Previous discovery: #{discovery_stats['previous_discovery_count'] || 61} files"
    puts "   Enhanced discovery: #{discovery_stats['legitimate_test_files'] || 0} files"
    puts "   Additional tests: +#{(discovery_stats['legitimate_test_files'] || 0) - (discovery_stats['previous_discovery_count'] || 61)}"
    puts "   Categories: #{discovery_stats['categories_discovered'] || 0}"
    puts
    ...
    puts "❌ Fixed test suite runner failed: #{e.class}: #{e.message}"
    puts "Backtrace:"
    e.backtrace&.each { |line| puts "  #{line}" }
    exit 2
  end
