...
# Enhanced Comprehensive Test Suite Runner with Dynamic Test Discovery
# Fixes test discovery issues and ensures all legitimate test files are included
...
class EnhancedComprehensiveTestSuiteRunner
  def initialize
  ...
  def run_comprehensive_suite
    log_header("🚀 ENHANCED COMPREHENSIVE TEST SUITE RUNNER")
    ...
    log_info("Enhanced dynamic test discovery enabled")
    log_separator

    begin
      Timeout::timeout(@total_timeout) do
        # Enhanced dynamic test discovery
  ...
  def discover_all_test_files
    log_info("🔍 Enhanced dynamic test discovery...")
    ...
    # Categorize test files dynamically based on directory structure
    ...
    log_info("📊 Dynamic categorization complete:")
  ...
  def categorize_test_files_dynamically(test_files)
  ...
  def run_single_test_file(test_file, category)
    ...
    relative_path = test_file.sub(@base_path + '/', '')
  ...
  def run_isolated_test(test_file)
    ...
    script_path = File.join(@base_path, 'temp_enhanced_test_runner.rb')
  ...
  def create_isolated_test_script(test_file)
    <<~RUBY
      # Enhanced isolated test runner script
      begin
        # Set up load paths
  ...
  def generate_coverage_analysis
    log_info("📊 Generating enhanced coverage analysis...")
    ...
    if defined?(SimpleCov) && SimpleCov.result
      ...
      elsif result.respond_to?(:total_branches) && result.total_branches > 0
  ...
  def analyze_results
    ...
    if discovery_stats['total_files_found'] > discovery_stats['legitimate_test_files']
      recommendations << {
        'priority' => 'high',
        'category' => 'discovery',
        'description' => "Enhanced discovery found #{discovery_stats['legitimate_test_files']} test files vs previous #{discovery_stats['total_files_found'] - discovery_stats['excluded_files']}",
        'action' => 'Test discovery has been enhanced to include all legitimate test files',
        'improvement' => "+#{discovery_stats['legitimate_test_files'] - (discovery_stats['total_files_found'] - discovery_stats['excluded_files'])} additional tests"
      }
    end
  ...
  def save_comprehensive_report
    log_info("💾 Saving enhanced comprehensive test report...")
    
    # Save JSON report
    json_report_path = File.join(@base_path, 'ENHANCED_COMPREHENSIVE_TEST_SUITE_REPORT.json')
    File.write(json_report_path, JSON.pretty_generate(@results))
    
    # Save human-readable report
    text_report_path = File.join(@base_path, 'ENHANCED_COMPREHENSIVE_TEST_SUITE_REPORT.md')
    File.write(text_report_path, generate_enhanced_markdown_report)
  ...
  def generate_enhanced_markdown_report
    report = []
    report << "# Enhanced Comprehensive Test Suite Report"
    report << ""
    ...
    if @results['coverage']['line_coverage']
      report << "## Enhanced Coverage Analysis"
      report << ""
  ...
  def display_final_summary
    log_separator
    log_header("📊 ENHANCED COMPREHENSIVE TEST SUITE SUMMARY")
    ...
    log_info("📁 Total Test Files: #{summary['total_files']} (was #{discovery['total_files_found'] - discovery['excluded_files']})")
    ...
    if @results['coverage']['line_coverage']
      log_separator
      log_info("📊 ENHANCED COVERAGE ANALYSIS:")
      cov = @results['coverage']
    ...
    log_separator
    log_info("📄 Enhanced reports saved:")
    log_info("   - ENHANCED_COMPREHENSIVE_TEST_SUITE_REPORT.json")
    log_info("   - ENHANCED_COMPREHENSIVE_TEST_SUITE_REPORT.md")
...
# Main execution
if __FILE__ == $0
  runner = EnhancedComprehensiveTestSuiteRunner.new
  ...
  begin
    ...
    discovery_stats = results['discovery_stats']
    puts
    puts "🎉 ENHANCED TEST DISCOVERY COMPLETE!"
    puts "   Previous discovery: #{discovery_stats['total_files_found'] - discovery_stats['excluded_files']} files"
    puts "   Enhanced discovery: #{discovery_stats['legitimate_test_files']} files"
    puts "   Additional tests: +#{discovery_stats['legitimate_test_files'] - (discovery_stats['total_files_found'] - discovery_stats['excluded_files'])}"
    puts
    ...
    puts "❌ Enhanced test suite runner failed: #{e.class}: #{e.message}"
    puts "Backtrace:"
    e.backtrace&.each { |line| puts "  #{line}" }
    exit 2
  end
